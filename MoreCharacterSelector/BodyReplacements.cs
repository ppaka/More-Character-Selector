...
namespace MoreCharacterSelector
{
            ...
            while (true)
            {
                if (Blink == true)
                {
                    foreach (var r in replacementModel.GetComponentsInChildren<SkinnedMeshRenderer>())
                    {
                        ...
                        if (blinkIndex2 != -1)
                        {
                            r.SetBlendShapeWeight(blinkIndex2, 100f);
                        }
                    }
                    ...
                    foreach (var r in replacementModel.GetComponentsInChildren<SkinnedMeshRenderer>())
                    {
                        var blinkIndex3 = r.sharedMesh.GetBlendShapeIndex("EyeClose");
                        if (blinkIndex3 != -1)
                        {
                            r.SetBlendShapeWeight(blinkIndex3, 0f);
                        }
                    }
    ...
    public class MRPEPE : BodyReplacementBase
    {
        ...
        protected override void OnDeath()
        {
            foreach (var r in replacementDeadBody.GetComponentsInChildren<SkinnedMeshRenderer>())
            {
                var blinkIndex6 = r.sharedMesh.GetBlendShapeIndex("CloseEye");
                if (blinkIndex6 != -1)
                {
                    r.SetBlendShapeWeight(blinkIndex6, 100f);
                    Blink = false;
                }
                else
                {
            ...
            while (true)
            {
                if (Blink == true)
                {
                    foreach (var r in replacementModel.GetComponentsInChildren<SkinnedMeshRenderer>())
                    {
                        ...
                        if (blinkIndex4 != -1)
                        {
                            r.SetBlendShapeWeight(blinkIndex4, 100f);
                        }
                    }
                    ...
                    foreach (var r in replacementModel.GetComponentsInChildren<SkinnedMeshRenderer>())
                    {
                        var blinkIndex5 = r.sharedMesh.GetBlendShapeIndex("CloseEye");
                        if (blinkIndex5 != -1)
                        {
                            r.SetBlendShapeWeight(blinkIndex5, 0f);
                        }
                    }
